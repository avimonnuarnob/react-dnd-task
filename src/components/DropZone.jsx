...
import { COMPONENT, SIDEBAR_ITEM, ROW, COLUMN } from '../lib/constants';
...
const DropZone = ({ data, onDrop, isLast, className }) => {
  const [{ isOver, canDrop }, drop] = useDrop({
    ...
    canDrop: (item, monitor) => {
      ...
      if (!itemPath) {
        if (item.component.type === COLUMN && splitDropZonePath.length > 2) {
          return false;
        } else if (
      ...
      // const sideBarParentDropInChild = item.component.type === data.dropZone;
